...
export class DeletionHandler implements RequestHandler {
  ...
  async execute(request: RequestMessage): Promise<void> {
    ...
    const [error] = await to((async () => {
      ...
      const getExpenseResult = JSON.parse(await getExpenseTool.invoke({ ledgerId, messageId }));

      let result: ResultMessage = { message: { channelId, messageId, requestId } };
      if (getExpenseResult.success) {
        const deleteExpenseResult = JSON.parse(await deleteExpenseTool.invoke({ ledgerId, messageId }));
        if (deleteExpenseResult.success) {
          result = { message: { result: "Expense deleted successfully.", channelId, messageId, requestId } };
        }
      }
